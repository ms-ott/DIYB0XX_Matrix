/*
  ...
  This code utilizes
    ...
    Mark Stanley & Alexander Brevig's Keypad library.
...
byte buttons[ROWS][COLS] = {
  {0, 1, 2, 3, 4, 5, 6}, //0 = AU, 1 = AL, 2 = AD, 3 = AR, 4 = Tilt, 5 = X, 6 = Mod1
  {7, 8, 9, 10, 11, 12, 13}, //7 = A, 8 = B, 9 = R, 10 = Z, 11 = L, 12 = Y, 13 = Extra1
  {14, 15, 16, 17, 18, 19, 20} //14 = Extra2, 15 = CU, 16 = CL, 17 = Light, 18 = CR, 19 = CD, 20 = Start
};

byte rowPins[ROWS] = {A0, A1, A2}; // row pins
byte colPins[COLS] = {2, 3, 4, 5, 6, 7, 8,}; //column pins

Keypad b0xx = Keypad( makeKeymap(buttons), rowPins, colPins, ROWS, COLS );
...
void setup()
{
  ...
  for (int i=0; i<LIST_MAX; i++)   //Poll the keylist
        {
            if ( b0xx.key[i].stateChanged )   //Only find buttons with a state change
            {
                switch (b0xx.key[i].kcode) {
                  case 19: //If cDown is held, set device to PC
                  currentDevice = PC;
                  break;
                  case 8: //If B is held, set game to Ultimate 
                  currentGame = Ultimate;
                  currentSOCD = TwoIP;
                  break;
                  case 5: //If X is held, set game to PM
                  currentGame = PM;
                  currentSOCD = TwoIPNoReactivate;
                  break;
                  case 9: //If R is held, set game to Rivals
                  currentGame = Rivals;
                  currentSOCD = TwoIP;
                }
            }
...
void loop()
{
  ...
  if (currentDevice == PC){
    Serial.print("B0XX is in PC Mode");
  }
   loopCount++;
